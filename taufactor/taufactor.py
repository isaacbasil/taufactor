...
class FullTensorSolver:
    ...
    def __init__(self, img, bc=(-0.5, 0.5), D_0=1, device=torch.device('cuda')):
        ...
        self.cb = self.init_cb(img)
        bs, x, y, z = self.cpu_img.shape
        self.L_A = torch.tensor([(x / (z * y)), (1 / z), (1 / y)]) # ISAAC x/xz simplifies to 1/z and likewise with x/xy
        # solving params
        self.converged = False
        self.semi_converged = False
        ...
        img = None
        # Results
        self.tau = None #TODO: make this into a vector, which has 3 components
        self.D_eff = None #TODO: make this into a vector, which has 3 components
        self.D_mean = None #TODO: make this into a vector, which has 3 components
    ...
    def check_vertical_flux(self, conv_crit):
        # ISAAC i think this is done in 2 different steps because conc is a different size to vert_flux, and the boolean mask needs to be the same size
        #calculate flux in x (vertical) direction
        vert_flux = self.conc[:, 1:-1, 1:-1, 1:-1] - \
            self.conc[:, :-2, 1:-1, 1:-1]  # ISAAC subtracts cell x from cell x+1 - flux values are defined at interfaces not in cells
        vert_flux[self.conc[:, :-2, 1:-1, 1:-1] == 0] = 0 # ISAAC at any interface, if one of the neighbouring cells is solid, the vertical flux will be 0
        vert_flux[self.conc[:, 1:-1, 1:-1, 1:-1] == 0] = 0  #ISAAC checking the second neighbouring cell

        # flux in y direction
        y_flux = self.conc[:, 1:-1, 1:-1, 1:-1] - \
            self.conc[:, 1:-1, :-2, 1:-1]
        y_flux[self.conc[:, 1:-1, :-2, 1:-1] == 0] = 0
        y_flux[self.conc[:, 1:-1, 1:-1, 1:-1] == 0] = 0

        # flux in z direction
        z_flux = self.conc[:, 1:-1, 1:-1, 1:-1] - \
            self.conc[:, 1:-1, 1:-1, :-2]
        z_flux[self.conc[:, 1:-1, 1:-1, :-2] == 0] = 0
